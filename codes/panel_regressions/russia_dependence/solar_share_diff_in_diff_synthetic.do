...
drop if bzone == "Switzerland" // Not in EU so missing Russian gas data
...
program define synth_did
    ...
    // ---------------------------------------------------------------
    collapse (mean) gas_share solar_share energy_price ln_solar_share temperature sun precipitation population_density gdp_pps russia_dependency, ///
    ...
    gen gas_russia = gas_share * russia_dependency
    label var gas_russia "gas_share × russia_dependency"
    gen gas_russia_scaled = gas_russia * 100
    label var gas_russia_scaled "gas_russia (scaled by 100 for synth)"
    ...
    preserve
        ...
        /* export delimited using ///
            "outputs/panel/solar_diff_and_diff/russia_dependence/synth_predictors_`tag'.csv", ///
            replace
        di as text "Predictor table saved: outputs/panel/solar_diff_and_diff/russia_dependence/synth_predictors_`tag'.csv" */
    restore
    ...
    synth solar_share ///
        ...
        gas_russia_scaled(1(1)`ref_pos'), ///
        trunit(`lv_id') trperiod(`trperiod_pos') ///
        customV(0.25 0.10 0.25 0.25 0.10 0.15) ///
        /*nested allopt*/
    ...
    gen _wt_gas_russia    = _synth_wt * gas_russia
    ...
    preserve
        ...
        collapse ///
            (sum)  synth_solar_share      = _wt_solar         ///
                   ...
                   synth_gas_russia       = _wt_gas_russia     ///
    ...
    drop _synth_wt _wt_solar _wt_ln_solar _wt_gas_russia _wt_temperature _wt_sun ///
    ...
    preserve
        collapse (mean) lv_solar      = solar_share        sc_solar      = synth_solar_share ///
                        lv_gas_russia = gas_russia          sc_gas_russia = synth_gas_russia, ///
            by(hy_seq_pos)
        ...
        twoway ///
            ...
            scheme(s2color)
        graph export "outputs/panel/solar_diff_and_diff/russia_dependence/synth_path_solar_share_`tag'.png", ///
            replace width(1400) height(900)

        // Gas × Russia dependency path
        twoway ///
            (connected sc_gas_russia period, ///
                mcolor(gs8) lcolor(gs8) msymbol(triangle) lpattern(dash) msize(small)) ///
            (connected lv_gas_russia period, ///
            ...
            xtitle("Half-year period") ///
            ytitle("Gas share × Russia dependency (half-year mean)") ///
            title("Latvia vs. synthetic: gas × Russia dependency [`tag']") ///
            subtitle("Red line = treatment start (`hy_lbl' `yr')") ///
            note("Synthetic control (Abadie et al. 2010).", size(vsmall)) ///
            scheme(s2color)
        graph export "outputs/panel/solar_diff_and_diff/russia_dependence/synth_path_gas_russia_`tag'.png", ///
            replace width(1400) height(900)
    restore
    ...
    drop synth_solar_share synth_ln_solar_share synth_gas_russia synth_temperature ///
    ...
    di as text "Done [`tag']. Outputs in outputs/panel/solar_diff_and_diff/russia_dependence/"
end
...
di as text ""
di as text "All done. Outputs saved to outputs/panel/solar_diff_and_diff/russia_dependence/"
