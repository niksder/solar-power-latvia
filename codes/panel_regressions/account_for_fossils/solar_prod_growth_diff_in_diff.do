...
preserve
    ...
    graph export "outputs/panel/solar_diff_and_diff/account_for_fossils/event_study_solar_production_growth.png", ///
        replace width(1400) height(900)

    // -------------------------------------------------------------------------
    // LATVIA-SPECIFIC EFFECT: coef × Latvia's pre-war gas share
    // Interpretation: extra daily solar log-growth Latvia gained (vs. zero-gas
    // counterfactual) relative to the H2 2020 reference period.
    // -------------------------------------------------------------------------
    ...
    // --- Plot 1: Growth rate per half-year ---
    twoway ///
        ...
        legend(off) ///
        xtitle("Half-year period") ///
        ytitle("Extra daily solar growth rate (log pts)") ///
        title("Latvia: extra daily solar growth rate attributable to pre-war gas dependence") ///
        subtitle("DiD coefs × Latvia gas share (`lv_gas_fmt' pp); ref = H2 2020; red = invasion") ///
        note("Each point = estimated extra daily log-diff in solar production for Latvia vs. zero-gas counterfactual." ///
             "Two-way FE (bzone + date). SE clustered at bzone level (N = 14 bzones).", size(vsmall)) ///
        scheme(s2color)

    graph export "outputs/panel/solar_diff_and_diff/account_for_fossils/event_study_latvia_growth_rate.png", ///
        replace width(1400) height(900)

    // --- Plot 2: Accumulated effect ---
    // Multiply each period's average daily effect by ~182 days to get the
    // log-point contribution for that half-year, then take the running sum.
    // This gives the total accumulated extra log solar production for Latvia
    // relative to a zero-gas-dependence counterfactual, starting from H1 2017.
    local days_per_hy = 182
    gen acc_coef = sum(coef * `days_per_hy')
    gen acc_lb95 = sum(lb95 * `days_per_hy')
    gen acc_ub95 = sum(ub95 * `days_per_hy')
    gen acc_lb90 = sum(lb90 * `days_per_hy')
    gen acc_ub90 = sum(ub90 * `days_per_hy')

    twoway ///
        (rarea acc_lb95 acc_ub95 period, color(maroon%15) lwidth(none)) ///
        (rarea acc_lb90 acc_ub90 period, color(maroon%25) lwidth(none)) ///
        (connected acc_coef period, ///
            mcolor(maroon) lcolor(maroon) msymbol(circle) lpattern(solid)), ///
        yline(0, lpattern(dash) lcolor(gray)) ///
        xline(2.5, lpattern(dash) lcolor(red) lwidth(medthick)) ///
        xlabel( ///
            ...
            -3 "H1 2019" -2 "H2 2019" -1 "H1 2020"  0 "H2 2020" ///
             ...
             9 "H1 2025" 10 "H2 2025", ///
            angle(45) labsize(small)) ///
        legend(order(3 "Accumulated effect" 2 "90% CI" 1 "95% CI") ///
               position(11) ring(0) cols(1)) ///
        xtitle("Half-year period") ///
        ytitle("Accumulated extra log solar production (log pts)") ///
        title("Latvia: accumulated solar production effect of pre-war gas dependence") ///
        subtitle("Cumulative DiD coefs × Latvia gas share (`lv_gas_fmt' pp) × ~182 days; red = invasion") ///
        note("Running sum of per-period log-point contributions (daily coef × ~182 days/period)." ///
             "Positive slope = Latvia's solar production growing faster than zero-gas counterfactual." ///
             "Two-way FE (bzone + date). SE clustered at bzone level (N = 14 bzones).", size(vsmall)) ///
        scheme(s2color)

    graph export "outputs/panel/solar_diff_and_diff/account_for_fossils/event_study_latvia_accumulated_effect.png", ///
        replace width(1400) height(900)
restore

di "Done. Outputs saved to outputs/panel/solar_diff_and_diff/account_for_fossils/"
