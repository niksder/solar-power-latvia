...
PLOT_OUTPUT_PATH = OUTPUTS_DIR + "/panel/others/gas_share_of_countries.png"
...
# --- Load data ---
df = pd.read_csv(MERGED_PANEL_DATA_PATH, usecols=['bzone', 'year', 'gas_production', 'total_generation'])
df = df[(df['year'] >= START_DATE.year) & (df['year'] <= END_DATE.year)]

# --- Compute annual gas share per country ---
annual = (
    df.groupby(['bzone', 'year'])[['gas_production', 'total_generation']]
    .sum()
    .reset_index()
)
...
table = annual.pivot(index='bzone', columns='year', values='gas_share')
table.index.name = 'Country'
table.columns.name = 'Year'

pct_table = (table * 100).round(2)
print("\nGas share of total electricity production (%) by country and year:")
...
ax.set_title('Gas share of total electricity production by country')
ax.set_xlabel('Year')
